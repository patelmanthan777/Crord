 ...
 */
class HighlightViewImp {

    private static final String TAG = HighlightViewImp.class.getSimpleName();
    ...
    private static final int DEFAULT_HIGHLIGHT_COLOR = 0xFFBDC0BA;
    ...
    public HighlightViewImp(View context) {
        viewContext = context;
    }

    public void setup(Rect imageRect, RectF cropRect) {
...
//        }
    ...
    void handleMotion(int edge, float dx, float dy) {
        if (edge == MOVE) {
            ...
            if (DBG) Log.i(TAG, "rect left " + cropRect.left + " top " + cropRect.top + " right " + cropRect.right + "bottom" + cropRect.bottom);

            // Convert to image space before sending to growBy()
            float xDelta = dx;
            float yDelta = dy;
            growBy(xDelta, yDelta, edge);
        }
    }
...
//        }
        ...
        if ((edge & GROW_LEFT_EDGE) == GROW_LEFT_EDGE) {
            r.set(r.left + dx, r.top, r.right, r.bottom);
        }
        ...
        if ((edge & GROW_TOP_EDGE) == GROW_TOP_EDGE) {
            r.set(r.left, r.top + dy, r.right, r.bottom);
        }

        if ((edge & GROW_BOTTOM_EDGE) == GROW_BOTTOM_EDGE) {
            r.set(r.left, r.top, r.right, r.bottom + dy);
        }
    ...
    public Rect getWindowCropRect() {
        int[] location = new int[2];
        viewContext.getLocationInWindow(location);

        Rect windowRect = new Rect((int) cropRect.left, (int) cropRect.top + location[1], (int) cropRect.right, (int) cropRect.bottom + location[1]);
        return windowRect;
    }

    // Maps the cropping rectangle from image space to screen space
    private Rect computeLayout() {
        return new Rect(Math.round(cropRect.left), Math.round(cropRect.top),
